    ...
    <Header>
      <HeaderContainer>
        <Logo to={`/profile/${currentUserId}`}>
          <img src={process.env.PUBLIC_URL + "/chronos.PNG"} alt="logo" />
          <h1>Chronos</h1>
        </Logo>
                ...
                <SearchResultsWrapper>
                  {findedEvents.length > 0
                    ? findedEvents.map((event, index) => (
                        <SearchResultsContent
                          key={index}
                          to={"mycalendar/" + event.calendarId}
                          onClick={searchResetter}
                        >
                          <SearchResultsContentText>
                            <strong>Calendar Name: {event.calendarName}</strong>
                            <br />
            ...
            {currentUserId !== 0 ? (
              <Tooltip title="Show profile" placement="bottom">
                ...
                >
                  ...
                  />
                  <Box sx={{ marginTop: "12px" }}>{userData.login}</Box>
                </StyledNavLink>
              </Tooltip>
            ) : (
